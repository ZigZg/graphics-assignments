...
SceneParser::parseLights() 
{
    ...
    while (_num_lights > count) {
        ...
        } else if(strcmp(token, "PointLight")==0)
        {
            ...
            _lights.push_back(parseSpotLight());
        } else if(strcmp(token, "RotatingSpotLight")== 0) {
            _lights.push_back(parseRotatingSpotLight());
        }
        else {
...
{
    //position, color, falloff, width, direction
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token); assert(!strcmp(token, "{"));
    ...
    float width = DegreesToRadians(readFloat());
    getToken(token); assert(!strcmp(token, "falloff"));
    float falloff = DegreesToRadians(readFloat());
    getToken(token); assert(!strcmp(token, "}"));
    return new SpotLight(position, direction, color, width, falloff);
}

Light *
SceneParser::parseRotatingSpotLight()
{
    //position, color, falloff, width, direction
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token); assert(!strcmp(token, "{"));
    ...
    getToken(token); assert(!strcmp(token, "falloff"));
    float falloff = DegreesToRadians(readFloat());
    getToken(token); assert(!strcmp(token, "rotation"));
    float rotation = DegreesToRadians(readFloat());
    getToken(token); assert(!strcmp(token, "}"));
    return new RotatingSpotLight(position, direction, color, width, falloff, rotation);
}
...
SceneParser::parseTransform() 
{
    ...
    while (true) {
        ...
        } else if (!strcmp(token,"ContinuousRotate")) {
            increment = DegreesToRadians(readFloat());
    ...
    getToken(token); assert(!strcmp(token, "}"));
    if (continuousRotate) {
        return new ContinuousTransform(matrix, object, increment);
    } else {
